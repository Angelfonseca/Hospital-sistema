...
export default function Reportes() {
    ...
    if (e.key === "Enter") {
      ...
      try {
        ...
        const respuesta = await clienteAxios.get(
          `/api/objects/codes/${SearchID}`
        );
        const NewProducto = respuesta.data[0];

        // Verificar si el producto ya está en el array
        const productoYaExiste = productos.some(
          (producto) => producto._id === NewProducto._id
        );
  ...
  const CambiarResponsable = async () => {
    ...
    try {
      for (let i = 0; i < productos.length; i++) {
        var Product =
          productos[i].asignado +
          "-" +
          productos[i].cve_cabms +
          "-" +
    ...
    try {
      await clienteAxios.post(
        `/api/objects/responsable/update/codes`,
        CambioResp
      );
  ...
  const GenerarExcel = async (e) => {
    ...
    setTimeout(async () => {
      ...
      const respuesta = await clienteAxios.post(`/api/objects/xlsx`, codes, {
        responseType: "blob",
      });
  ...
  return (
    <>
      ...
      <div className="Container_Reports">
        <div>
          <Link to="/">
            <button id="btnRegresarRep">Regresar</button>
          </Link>
        </div>
        <div>
          <input
            ...
            onKeyPress={getProductByIDReport}
          />
          <select
            name="Responsable"
            className="Selector"
          ...
          <table id="TableProducts">
            ...
            <tbody>
              {productos.map((producto) => (
                <tr key={producto._id}>
                  <td data-titulo="Idetificador">
                    {producto.asignado}-{producto.cve_cabms}-
